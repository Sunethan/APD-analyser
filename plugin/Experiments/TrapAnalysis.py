...
class CurrentFitting(object):
    ...
    def PlotIV(self, T, material, type, parameter):
        if material == 'InP':
            ...
            V_InGaAs = np.asarray([V for V in self.RawIV[T].X if -self.T_analysis_v_max[T] <= V <= -self.v_min['InGaAs']])
            F_InGaAs = np.asarray([self.Em_InGaAs(V) for V in V_InGaAs])
            if type == 'SRH':
                def SRH_InGaAs_IV(X, Eti, tp, tn):
                    Emax_Vcm, T = X
    ...
    def Plotdm(self, T, material):
        if material == 'InP':
            V_InP = np.asarray([V for V in self.RawIV[T].X if -self.v_min['InP'] >= V > -self.v_max['InP']])
            F_InP = np.asarray([self.Em_InP(V) for V in V_InP])
            return V_InP, self.dm_InP(F_InP, self.ND, self.Ncharge, self.d_mul, self.d_ch)
        elif material == 'InGaAs':
            V_InGaAs = np.asarray([V for V in self.RawIV[T].X
                                   if -self.T_analysis_v_max[T] <= V <= -self.v_min['InGaAs']])
            F_InGaAs = np.asarray([self.Em_InGaAs(V) for V in V_InGaAs])
            return V_InGaAs, self.dm_InGaAs(F_InGaAs, self.ND_abs, self.d_InGaAs)
        else:
            raise BaseException("Wrong material: %s" % material)
    ...
    def plot_current_electric_field(self, number, figtitle):
        ...
        for i, T in enumerate(self.T_analysis):
            ...
            raw_I = [abs(element) for i, element in enumerate(self.RawIV[T].Y) if i % count == 0]
            plt.plot(raw_V, raw_I, linestyle='none', marker='o', fillstyle='none', color=ColorSet10[i], label='%.0f' % T)
        ...
        plt.legend(loc='best')
        plt.subplot(1, 2, 2)
        plt.plot(*self.PlotEm(self.room_temperature(), 'InP'), color='b', label='InP')
        plt.plot(*self.PlotEm(self.room_temperature(), 'InGaAs'), color='r', label='InGaAs')
    ...
    def plot_trap_gammaDx(self, number, figtitle, filename=None):
        ...
        for i, item in enumerate(self.material):
            for j, model in enumerate(self.mechanism):
                plt.plot(self.T_analysis, [self.result['IV'][item][model][T][3] for T in self.T_analysis], marker='o',
                         fillstyle='none', linestyle=LineSet2[j], color=ColorSet10[-1 - i], label=r'[IV][%s][%s]' % (item, model))
                if 'IT' in self.method:
                    plt.plot(self.T_analysis, [Eti_avg[item][model] for T in self.T_analysis], color=ColorSet10[-3 - i],
        ...
        for i, T in enumerate(self.T_analysis):
            for j, item in enumerate(self.material):
                if item == 'InP':
                    plt.plot(*self.d_Gamma(T, item), color=ColorSet10[i], label='%.0f K' % T)
                else:
                    plt.plot(*self.d_Gamma(T, item), color=ColorSet10[i])
                SaveData['%s-%s(K)-V-(TAT)' % (item, T)] = self.d_Gamma(T, item)[0]
                SaveData['%s-%s(K)-d-(TAT)' % (item, T)] = self.d_Gamma(T, item)[1]
        plt.plot(*self.Plotdm(self.room_temperature(), 'InP'), color='k', linestyle='-.', label='Depletion\nwidth')
        plt.plot(*self.Plotdm(self.room_temperature(), 'InGaAs'), color='k', linestyle='-.')
        SaveData['InP-V'] = self.Plotdm(self.room_temperature(), 'InP')[0]
        SaveData['InP-d'] = self.Plotdm(self.room_temperature(), 'InP')[1]
        SaveData['InGaAs-V'] = self.Plotdm(self.room_temperature(), 'InGaAs')[0]
        SaveData['InGaAs-d'] = self.Plotdm(self.room_temperature(), 'InGaAs')[1]
        if filename is not None:
            utils.save_multiple_arrays(filename, SaveData)
        plt.grid()
        plt.xlabel('Voltage (V)')
        plt.ylabel(r'$d$ (cm)')
        ...
        #plt.xlim((-25, 2))
        plt.yscale('log')
        plt.legend(loc='upper right', ncol=2, bbox_to_anchor=(0.6, 0.825))
    ...
    def plot_lifetime_IV(self, number, figtitle):
        ...
        for i, item in enumerate(self.material):
            plt.subplot(2, 2, i + 1)
            for model in self.mechanism:
                plt.plot(self.T_analysis, [1e9 * self.Lifetime[item][model][T][0] for T in self.T_analysis], marker='o',
                         fillstyle='none', linestyle='-', label=r'$\tau_p$[%s]' % model)
            plt.plot(300, self.Lifetime_ref[item][1]['p'], color='k', marker='o', fillstyle='none',
                     markersize=12, linestyle='none', label=self.Lifetime_ref[item][0])
            plt.grid(True)
            plt.legend(loc='best')
            plt.title(item)
            ...
            plt.subplot(2, 2, i + 3)
            for model in self.mechanism:
                plt.plot(self.T_analysis, [1e9 * self.Lifetime[item][model][T][1] for T in self.T_analysis], marker='o',
                         fillstyle='none', linestyle='-', label=r'$\tau_n$[%s]' % model)
            plt.plot(300, self.Lifetime_ref[item][1]['n'], color='k', marker='o', fillstyle='none',
                     markersize=12, linestyle='none', label=self.Lifetime_ref[item][0])
            plt.grid(True)
            plt.legend(loc='best')
    ...
    def plot_lifetime_IT_fitting(self, number, figtitle):
        ...
        for material in self.material:
            for model in self.mechanism:
                if material == 'InP':
                    plt.plot(self.V_InP, [self.Lifetime_p['InP'] * self.result['IT'][material][model][V][3][0] for V in self.V_InP],
                             color=ColorModel[model], label=model)
                else:
                    plt.plot(self.V_InGaAs,
                             [self.Lifetime_p['InGaAs'] * self.result['IT'][material][model][V][3][0] for V in self.V_InGaAs],
                             color=ColorModel[model])
        plt.grid()
        plt.yscale('log')
        ...
        for material in self.material:
            for model in self.mechanism:
                if material == 'InP':
                    plt.plot(self.V_InP, [self.Lifetime_n['InP'] * self.result['IT'][material][model][V][3][1] for V in self.V_InP],
                             color=ColorModel[model], label=model)
                else:
                    plt.plot(self.V_InGaAs,
                             [self.Lifetime_n['InGaAs'] * self.result['IT'][material][model][V][3][1] for V in self.V_InGaAs],
                             color=ColorModel[model])
        plt.grid()
        plt.yscale('log')
        plt.xlabel(r'Voltage (V)')
    ...
    def plot_lifetime_IT_IV_comparison(self, number, figtitle):
        def lifetime_p(T, tp, alpha_p, material):
            tau_p = tp * self.Lifetime_p[material] * 1e-9 * (T / self.room_temperature()) ** alpha_p
            return tau_p

        def lifetime_n(T, tn, alpha_n, material):
            tau_n = tn * self.Lifetime_n[material] * 1e-9 * (T / self.room_temperature()) ** alpha_n
            return tau_n
        ...
        for i, material in enumerate(self.material):
            ...
            for j, model in enumerate(self.mechanism):
                ...
                for V in self.material_voltage[material]:
                    ...
                    if 'IV' not in self.method:
                        if LabelMarker1 == 0:
                            plt.plot(self.T_analysis_IT, 1e9 * self.Lifetime_p_DB[material][model][V],
            ...
            #plt.plot(300, self.Lifetime_ref[material][1]['p'], color='k', marker='o', fillstyle='none',
            #         markersize=12, linestyle='none', label=self.Lifetime_ref[material][0])
            if material in self.TCAD_lifetime and self.TCAD_check == 'Yes':
                plt.plot([T for T in self.T_analysis if T in self.TCAD_lifetime[material]],
                         [1e9 * self.TCAD_lifetime[material][T]['p'] for T in self.T_analysis if T in self.TCAD_lifetime[material]],
            ...
            for j, model in enumerate(self.mechanism):
                ...
                for V in self.material_voltage[material]:
                    ...
                    if 'IV' not in self.method:
                        if LabelMarker2 == 0:
                            plt.plot(self.T_analysis_IT, 1e9 * self.Lifetime_n_DB[material][model][V],
                                     color=ColorModel[model], linestyle=':', label=model)
                            LabelMarker2 = 1
                        else:
                            plt.plot(self.T_analysis_IT, 1e9 * self.Lifetime_n_DB[material][model][V],
                ...
                if 'IV' in self.method:
                    ...
                    plt.plot(self.T_analysis_IT, 1e9 * Lifetime_avg_n, color=ColorModel[model], marker='o',
                             fillstyle='none', label=r'[IT][%s]' % model)
            #plt.plot(300, self.Lifetime_ref[material][1]['n'], color='k', marker='o', fillstyle='none',
            #         markersize=12, linestyle='none', label=self.Lifetime_ref[material][0])
            if material in self.TCAD_lifetime and self.TCAD_check == 'Yes':
                plt.plot([T for T in self.T_analysis if T in self.TCAD_lifetime[material]],
                         [1e9 * self.TCAD_lifetime[material][T]['n'] for T in self.T_analysis if
    ...
    def plot_current_fitting_comparison(self, number, figtitle):
        ...
        plt.plot(1 / self.T_analysis_IT, np.log10(abs(IT_InP)), linestyle='none', marker='o', markersize='15',
                 fillstyle='none', label=r'$%s$ (V)' % self.V1)
        plt.plot(1 / self.T_analysis_IT, np.log10(abs(IT_InGaAs)), linestyle='none', marker='o', markersize='15',
    ...
    def plot_any(self, number, figtitle):
        ...
        if material in self.TCAD_lifetime and self.TCAD_check == 'Yes':
            plt.plot([T for T in self.T_analysis if T in self.TCAD_lifetime[material]],
                     [1e9 * self.TCAD_lifetime[material][T]['p'] for T in self.T_analysis if
                      T in self.TCAD_lifetime[material]],
                     marker='s', markersize=8, color='g', label='TCAD')
        plt.grid()
        plt.ylabel(r'$\tau_p$ (ns)')
    ...
    def plot_effective_mass_comparison(self, number, figtitle):
        ...
        if 'InP' in self.material:
            ...
            plt.xlabel('Voltage (V)')
            plt.ylabel('Current (A)')
            plt.title(r'$R_{TAT}\approx(1+\Gamma_%s)R_{SRH}$ ; $%s=%sm_0$' % (
            Gamma_type['InP'], mass_type['InP'], self.effective_mass_InP))
            plt.xlim((-30, 1))
            plt.ylim((1e-14, 1e-8))
            plt.legend(loc='best')

        if 'InGaAs' in self.material:
            LabelMarker = 0
            temp = None
            plt.subplot(1, 2, 2)
            for i, T in enumerate(self.T_analysis):
            ...
            plt.ylabel('Current (A)')
            plt.title(r'$R_{TAT}\approx(1+\Gamma_%s)R_{SRH}$ ; $%s=%sm_0$' % (
            Gamma_type['InGaAs'], mass_type['InGaAs'], self.effective_mass_InGaAs))
            plt.xlim((-45, -20))
            plt.ylim((1e-12, 1e-6))
            plt.legend(loc='best')
